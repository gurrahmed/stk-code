...
// ----------------------------------------------------------------------------
/** Places a new item on the track. All items are converted to big nitro
 *  to ensure only nitro pickups appear in network games. */
Item* NetworkItemManager::placeItem(ItemState::ItemType type,
                                    const Vec3& xyz,
                                    const Vec3& normal)
{
    if (type != ItemState::ITEM_NITRO_SMALL)
        type = ItemState::ITEM_NITRO_BIG;
    return ItemManager::placeItem(type, xyz, normal);
}   // placeItem
...
void NetworkItemManager::restoreState(BareNetworkString *buffer, int count)
{
    ...
    while(count > 0)
    {
        ...
        {
            AbstractKart *kart = world->getKart(iei.getKartId());
            ItemState::ItemType new_type = iei.getNewItemType();
            if (new_type != ItemState::ITEM_NITRO_SMALL)
                new_type = ItemState::ITEM_NITRO_BIG;
            ItemState *is = new ItemState(new_type, kart, iei.getIndex());
            is->initItem(new_type, iei.getXYZ(), iei.getNormal());
            if (m_switch_ticks >= 0)
            {
...
void NetworkItemManager::restoreCompleteState(const BareNetworkString& buffer)
{
    ...
    for (unsigned i = 0; i < all_items; i++)
    {
        ...
        if (has_item)
        {
            ...
            if (is->getType() != ItemState::ITEM_NITRO_SMALL)
                is->setType(ItemState::ITEM_NITRO_BIG);
            m_confirmed_state.push_back(is);
        }
        else
            m_confirmed_state.push_back(NULL);
